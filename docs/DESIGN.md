...
> as "Release 2" of [ran-twin](https://github.com/aether-gui/ran-twin) (the
> UERANSIM-based 5G twin) and inherits that plan; it now lives and evolves on its
> own. Where this document says "ran-twin," it means that sibling project — the
> existing scenario-driven twin ORBIT interoperates with but does not depend on.
> Reuse of ran-twin's `internal/*` packages therefore means **copy, or promote the
...
```
                        ran-twin (single Go binary; roles selectable)
┌──────────────────────────────────────────────────────────────────────────────┐
...
**Relationship to the ran-twin orchestrator.** ran-twin's `internal/twin` (`twin.go`, `run.go`) is deeply UERANSIM-specific — `docker exec`, `nr-cli`, per-cell container management. We **keep and extend** the portable packages — `internal/scenario`, `internal/config`, `internal/traffic` — and **replace `twin`** with the new engine behind an `EngineBackend` interface, so UERANSIM stays optionally swappable. **Correction from the draft:** the "contiguous-IMSI" and "one-`nr-ue`-process-per-cell" constraints exist **only as doc comments** (`scenario.go` line 7 and the comment above `validate()`), **not as executable checks** — `validate()` (lines 150–208) enforces cell/NCI/gnbId uniqueness, 15-digit numeric IMSIs, profile/cell references, and duration parsing, and nothing else. So the ORBIT task is to **correct the misleading comments**, not "relax validators," and to **re-derive what actually blocks ran-twin scenarios on the native engine** (§6 Phase 2). **ran-twin single-slice scenarios must load and run unchanged against the ORBIT engine** (a phase exit criterion; multi-slice is a schema extension, not ran-twin compat — see §5(g)).
...
Keep `internal/scenario`, `internal/config`, `internal/traffic`; introduce an `EngineBackend` interface so ran-twin's UERANSIM path stays swappable and ORBIT's native engine is the new default. Do **not** reuse `twin.Twin`/`twin.Run` (docker/nr-cli-bound). **The "relax the contiguous-IMSI / one-process-per-cell validators" task in the draft is void** — those checks do not exist in `validate()`; only misleading doc comments do. Replace those comments; independently identify what genuinely blocks ran-twin scenarios (config single-slice assumption, TrafficProfile mapping, fleet expansion). **Exit criterion:** existing ran-twin single-slice scenarios (`poc.yaml`, `multicell.yaml`, `scale-run.yaml`) run unchanged on the ORBIT engine.

### (c) RRC & XnAP: wire-encode now, or in-process stub? — **STUB in-process first. Decided; highest-leverage call, single-point-gated.**
Because the AMF passes RRC containers **opaquely** and never touches XnAP, the NGAP portion of a full handover against the *real* core can be validated with RRC and XnAP as **Go structs on channels**. Full TS 38.331 UPER / TS 38.423 APER codecs become mandatory only for inter-vendor interop or running two ran-twin gNBs as *separate processes*. **This decision rests entirely on D-3, and D-3's framing is refined:** it must confirm (1) the AMF is opaque to *RRC bytes*, **and** (2) `free5gc/ngap` (or the omec fork) correctly encodes the NGAP-level Source-to-Target Transparent Container that *wraps* the RRC blob, because our own target-side decode depends on it. Budget a contingency (partial NGAP-container encoding work) if (2) surprises.
...
**Verification:** unit-CI green headless; `ran-twin serve` starts, CLI reaches it over Connect; an SCTP association opens to the AMF's N2 port and **NG Setup** is exchanged (no UE yet), PPID asserted = 60.
...
3. Wire the `EngineBackend` behind `internal/scenario`; **correct the misleading doc comments** (no validator relaxation needed — §5b); confirm the real ran-twin blockers and fix them.
4. **Slicing schema extension (§5g):** add per-UE Requested NSSAI + per-session DNN/S-NSSAI to the scenario schema; enable **multi-PDU-session per UE** (up to 15, TS 23.501 §5.8.2.2). Keep single global `Core.Slice`/`Core.DNN` as the default so ran-twin scenarios still parse.
5. Introduce **Mode B** (shared-TUN demux) so scale does not require 10k TUN devices.
**Verification:** ran-twin **single-slice** `multicell.yaml`/`scale-run.yaml` load and run on the ORBIT engine; register N UEs across ≥2 gNBs on the live core (checkpoint: the SD-Core-validated **5,000 UEs @ 10 attach/s**); **sim-capacity** (goroutine/RSS scaling to 10k) measured separately against the mock AMF; memory ≈ ran-twin's ~2 MB/UE order or better.
**Exit:** stable multi-cell, multi-session fleet on the real core; ran-twin single-slice backward-compat proven; actor model chosen on D-6 evidence; Mode B available for scale.
...
| **Phantom validator task** — draft budgeted work to "relax" checks that don't exist. | Misspent effort; misread foundation undercuts ran-twin-compat confidence. | **Corrected (§5b):** fix misleading doc comments; independently re-derive real ran-twin blockers (config single-slice, traffic mapping, fleet expansion). |
...
| **Slicing needs unflagged schema work; "run unchanged" overclaimed.** | Multi-session/slicing silently blocked; ran-twin-compat claim too broad. | **§5g:** per-UE NSSAI + per-session DNN/S-NSSAI is an explicit Phase-2 task; "run unchanged" scoped to single-slice ran-twin scenarios; slicing gets Phase-7 acceptance criteria. |
...
**Relevant repo paths:** `/home/ben/repos/bengrewell/ran-twin/internal/scenario/scenario.go` (schema to preserve/extend; **doc comments at line 7 and above `validate()` are misleading — no contiguous-IMSI / one-process-per-cell checks exist in `validate()`, lines 150–208; correct the comments, add per-UE NSSAI + per-session DNN/S-NSSAI for §5g**); `/home/ben/repos/bengrewell/ran-twin/internal/config/config.go` (**single global `Core.Slice`/`Core.DNN` at lines 38–39, 54–57 — the slicing-extension baseline**); `/home/ben/repos/bengrewell/ran-twin/internal/twin/{twin.go,run.go}` (UERANSIM-specific, replaced behind `EngineBackend`); `/home/ben/repos/bengrewell/ran-twin/internal/{traffic,config,ueransim,oci}` (traffic/config keep; ueransim/oci become one backend); `/home/ben/repos/bengrewell/ran-twin/docs/DESIGN.md` (ran-twin design of record); `/home/ben/repos/bengrewell/ran-twin/cmd/ran-twin/main.go` (CLI entry to re-point at the Connect client).
