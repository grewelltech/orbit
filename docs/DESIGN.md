...
| **D-4** | What GTP-U **SN values** does the UPF accept post-handover? Does it honor End Markers? | **M-1 / P3** | In a PHY-less sim SN values are synthetic; a strict UPF could drop packets → continuity illusory. | iperf3/native flow through a handover; measure loss with/without End Marker and varied synthetic SNs. **PARTIAL 2026-07-04 — DL path switch does NOT take effect on SD-Core.** After a completed N2 handover: (a) baseline flow through the source works; (b) uplink from the *target* still reaches the UPF (UL TEID unchanged, accepted); but (c) the UPF keeps sending **downlink to the source** gNB — the SMF's DL FAR `OuterHeaderCreation` stays `172.17.50.12/TEID 1` (source) after the handover-completed modify. **Our HandoverRequestAcknowledge DL tunnel is correctly encoded (round-trip unit test),** so the gap is core-side (AMF→SMF→UPF handover DL switch), not our NGAP. omec's handover DL switch is unexercised (gnbsim never ran handover) → **candidate Phase-6 conformance/regression finding.** Full isolation would capture the N4/PFCP FAR-update. So mobility *signalling* is fully proven; user-plane *continuity* across handover is blocked by the core here. |
