...
**Status — N2 handover COMPLETES end-to-end on SD-Core (2026-07-04):** built the full source + target NGAP FSM (HandoverRequired → HandoverRequest → HandoverRequestAcknowledge → HandoverCommand → HandoverNotify) with RRC as opaque placeholder containers (D-3 stub validated). Run from the RAN node with distinct routed source IPs per gNB. AMF logs confirm the whole procedure incl. `send UE Context Release Command` + `detached UeContext from OldRanUe` — SD-Core drives the complete handover and releases the source. **Operational gotcha:** the omec AMF does not cleanly handle a gNB ID re-registering from a new address — a stale association causes `Ran addr is nil` on the target send; use fresh gNB IDs per run (a Phase-6 robustness/regression candidate). **Measurement-synthesis engine DONE (2026-07-04):** `internal/meas` implements A3/A4/A5 measurement-report events (TS 38.331 §5.5.4.4–.6) with hysteresis + time-to-trigger, a log-distance RSRP model, and a Scenario runner that turns synthesized UE motion into an ordered handover sequence; `engine.MobilityController` ties triggers to a `HandoverExecutor` interface (offline decision ⟂ core-bound execution). So simulated mobility now *drives* handover targets — unit-tested (single/chained/stationary). Remaining: D-4 data continuity (blocked by SD-Core Finding 2 — see `docs/interop/sdcore.md`), a live `HandoverExecutor` wiring meas → the N2 FSM, and API/StateStream exposure.
