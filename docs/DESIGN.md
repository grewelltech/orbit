...
### (i) Core compatibility: strict-by-default codecs + an opt-in quirks layer — **Decided (2026-07-04).**
**Problem shape.** ORBIT is a *general* multi-core tool, but real deployed cores are not perfectly 3GPP/X.691-conformant. The first concrete instance (D-4): SD-Core's SMF (omec's aper fork) rejects our **canonical, spec-conformant** `HandoverRequestAcknowledgeTransfer` with "align Bit is not zero" — verified independently against `pycrate` that *we* are conformant and the *core* is buggy. We must be able to drive such a core **without** making ORBIT itself non-conformant (which would break it against conformant cores — the whole risk).

**Decision — a two-layer model:**
1. **Codecs stay strict and conformant.** Never fork the shared `free5gc` `ngap`/`nas`/`aper`, and never bend our encoding to a specific core. Conformance is the invariant. If we ever find a genuine bug in *our* encode path (oracle says we're the non-conformant one), fix it there and upstream it — it helps every core.
2. **A named `CoreProfile` with an opt-in `QuirkSet`**, applied as a thin transform at the message-build/encode boundary (`internal/gnb`/`internal/ue`), **never inside the codec**. Shape:
   - `CoreProfile{ Name string; Quirks QuirkSet }`; profiles registered by name — `strict-3gpp` (**default, zero quirks**), `sdcore`, `free5gc`, …
   - `QuirkSet` is a struct of typed, individually-toggleable flags. Each quirk is **named, documented (the exact non-conformance + core + version + upstream link), spec-cited, and defaults off**. E.g. `HandoverAckTransferOmecAlign` → emits the byte layout omec's buggy decoder accepts, for SD-Core rel-<x>.
   - The profile is carried in config and threaded to the builders; a builder consults the relevant quirk and, only if set, substitutes the workaround encoding.

**Principles that keep it honest (not "tuning for SD-Core"):**
- **Strict by default** — a conformant core needs no profile and gets byte-exact 3GPP.
- **Opt-in and named** — quirks are never silent; you choose `--core-profile sdcore` knowingly.
- **Observable as a feature** — ORBIT *reports which quirks a core required to interoperate*. That list is a **conformance scorecard** (feeds Phase 6), not hidden debt.
- **Spec-cited & removable** — each quirk documents the defect it works around and is deleted when the core is fixed.
- **Upstream-first** — every quirk corresponds to a filed upstream bug; the workaround is the bridge, not the resolution.

This is the standard interop-tool pattern (TLS per-peer workarounds, browser quirks modes). It makes ORBIT usable against imperfect real cores *because* being a conformance/integration tester means meeting non-conformant peers — while the strict default keeps it a general, portable tool. **Introduce the scaffold when the first quirk is needed (the D-4 SD-Core handover workaround); until then the only profile is `strict-3gpp`.**
...
**Status — N2 handover COMPLETES end-to-end on SD-Core (2026-07-04):** built the full source + target NGAP FSM (HandoverRequired → HandoverRequest → HandoverRequestAcknowledge → HandoverCommand → HandoverNotify) with RRC as opaque placeholder containers (D-3 stub validated). Run from the RAN node with distinct routed source IPs per gNB. AMF logs confirm the whole procedure incl. `send UE Context Release Command` + `detached UeContext from OldRanUe` — SD-Core drives the complete handover and releases the source. **Operational gotcha:** the omec AMF does not cleanly handle a gNB ID re-registering from a new address — a stale association causes `Ran addr is nil` on the target send; use fresh gNB IDs per run (a Phase-6 robustness/regression candidate). **Measurement-synthesis engine DONE (2026-07-04):** `internal/meas` implements A3/A4/A5 measurement-report events (TS 38.331 §5.5.4.4–.6) with hysteresis + time-to-trigger, a log-distance RSRP model, and a Scenario runner that turns synthesized UE motion into an ordered handover sequence; `engine.MobilityController` ties triggers to a `HandoverExecutor` interface (offline decision ⟂ core-bound execution). So simulated mobility now *drives* handover targets — unit-tested (single/chained/stationary). **Live executor + API wiring DONE (2026-07-05):** `Manager.Handover` drives the full N2 FSM against the core (target association brought up on demand) and moves the session; `Manager.MobilityExecutor` turns meas triggers into live handovers; the `UEService.Handover` RPC + `orbit ue handover` CLI drive it, and mobility states (`HANDOVER_STARTED`/`HANDED_OVER`) stream over `StateStream`. Core profile is selectable (`serve --core-profile sdcore`). **Verified live against SD-Core:** register a UE via the API, `ue handover` to a target gNB, observe `HANDOVER_STARTED → HANDED_OVER` on the stream with the session preserved — **mobility driven and observed via API, end to end.** Only open item: D-4 user-plane continuity, blocked by SD-Core Finding 2 (`docs/interop/sdcore.md`).
...
**Advances:** mob, obs, api.
**Status — D-2 PASSED, Xn PathSwitch + data continuity verified live (2026-07-06).** `internal/gnb/xn.go` builds the NGAP `PathSwitchRequest` (source AMF-UE-NGAP-ID, target RAN-UE-NGAP-ID, UE security caps, per-session `PathSwitchRequestTransfer` with the target's new DL tunnel) + classify/parse helpers. Live: the AMF `Acknowledge`s and — unlike N2 — the SMF decodes the transfer cleanly and **programs the target tunnel into the UPF (N4-confirmed), so a flow survives the switch** (`TestLivePathSwitch`, `TestLivePathSwitchDataContinuity`). **XnAP preparation is stubbed in-process** (source↔target gNB coordination needs no wire in a single-process sim), so **D-9 (real XnAP codec) is not needed** unless going multi-process/inter-vendor. **Xn wiring DONE (2026-07-06):** `Manager.XnHandover` (brings up the target association, sends PathSwitchRequest, moves the session on Acknowledge, emits the `HANDOVER_*` StateStream events with an "Xn" detail) + `UEService.XnHandover` RPC + `orbit ue xn-handover`. **Verified live via the API:** register → `ue xn-handover` → `HANDOVER_STARTED → HANDED_OVER (via Xn)` on the stream, and a ping through the target after the switch returns **3/3 (4.9 ms)** — mobility with data continuity, driven and observed via the API. **Phase 4 complete** — Xn is a first-class capability alongside N2, and it delivers the user-plane continuity N2 cannot on SD-Core.
...
**Status — harness DONE (2026-07-06); D-11 resolved (free5gc decode path).** `internal/conformance`: `Test{ID,Category,SpecRef,Run}→Result{verdict,expected,observed,detail}` (JSON-tagged), a `Registry` (per-check timeout, a distinct gNB ID per check, category filter), `Env` with a **crash-safety liveness primitive** (`Env.Alive` — confirms the AMF still completes a fresh NG Setup after a negative test), `Summarize`/`Summary.OK` as the CI gate, and `orbit conformance --amf … [--json] [--category]` (non-zero exit on fail — the integration-CI entrypoint). **Four checks ship** (2026-08-01): `NGAP-UNKNOWN-UE-SURVIVES` (TS 38.413 §8.7.1/§10.6), a crash-safety regression guard for the omec #672/#673 class — **live PASS** (core survives; Error Indication logged as spec-ideal, graceful drop accepted — deliberately *not* overstated as a spec violation); `NGAP-NGSETUP-MISSING-TALIST` (negative-IE, live PASS: NG SETUP FAILURE); `NGAP-NG-RESET-ACK` (procedural); and `GTPU-UNKNOWN-TEID-ERRIND` (TS 29.281 §7.3.1 — scored DEVIATION, not FAIL; see interop/sdcore.md Finding 3). **Still deferred: malformed-IE on RANConfigurationUpdate/PathSwitch, and NAS replay** — they require careful per-message spec-walking to assert correctly (the crying-wolf risk the framing corrections warn about), best done deliberately against the TS rather than cranked out. Note the coverage map's ✅ marks include procedures merely *exercised* by normal operation, so it reads richer than the four authored checks. The harness makes each a small, uniform addition, walked against the coverage checklist ([docs/conformance/coverage.md](conformance/coverage.md)). `pcapFrame` correlation and SARIF output remain future niceties.
...
### Beyond P7 — future directions

Not committed work; recorded so the intent isn't lost and the honest limits are explicit.

- **Conformance coverage as a walked checklist.** The suite grows against a systematic map, not ad hoc: [docs/conformance/coverage.md](conformance/coverage.md) enumerates NGAP procedures (clause 8) × the clause-10/criticality negative matrix, tracks what's covered, and cites each check. New interfaces (XnAP, NAS-5GS, GTP-U) get their own sections as coverage extends.
- **Release-awareness (not multi-release exhaustiveness).** Every check is tagged with the 3GPP Release + TS version it asserts against; the suite baseline tracks the core under test (confirm SD-Core's omec release before asserting release-specific behaviour). Cross-release deltas are mostly additive, so release-specific checks `SKIP` on older cores rather than failing them. **Hard limit:** decode/encode is bounded by the free5gc codec's release coverage (~Rel-15/16, some Rel-17) — **Rel-18** messages/IEs need codec work *first*, so broadening the range is gated on that, not just on writing checks.
- **Network-under-test (testing a real gNB / a real UE).** Today the device under test is the *core* (ORBIT plays gNB+UE). The mirror axis — ORBIT playing the network to test a **real gNB's** N2/N3 conformance — is the natural extension and largely reuses the existing NGAP/NAS/GTP-U codecs from the other side (the `internal/mockamf` core stub is a seed). Testing a **real UE / device**, however, requires the **Uu radio interface (RRC + PHY)**, which is ORBIT's explicit non-goal — that is a fundamentally different, RF/channel-emulator class of tool (where the VIAVI/Spirent *hardware* lives) and would be a separate effort, not a PHY-less extension. Roadmap the gNB-facing direction; treat device/UE radio testing as out of scope for this tool.
...
| **D-2** | Does the core complete **Xn** `PathSwitchRequest`? | before P4 | Xn also "pending" in gnbsim; SD-Core 1.3 frames multi-gNB as groundwork. | Minimal PathSwitchRequest; confirm Ack + N4/N3 completion. **RESOLVED 2026-07-06 — POSITIVE.** Attached a UE with a PDU session on a source gNB, then sent a well-formed NGAP `PathSwitchRequest` from a second (target) gNB naming the UE by its source AMF-UE-NGAP-ID with the target's new DL tunnel. **AMF replied `PathSwitchRequestAcknowledge` — SD-Core completes the Xn path switch.** Crucially the SMF decoded the `PathSwitchRequestTransfer` with **zero align-bit errors** (processed the N2 SM info cleanly), unlike the N2 `HandoverRequestAcknowledgeTransfer` which hit omec's dropped-`optional` bug (Finding 2) — so **Xn achieves the user-plane continuity N2 could not — CONFIRMED 2026-07-06.** With a working N3 path, after the PathSwitch a ping through the *target* tunnel succeeds, and an N4/PFCP capture shows the SMF programmed the target tunnel (`172.17.50.13`/TEID `0x300`) into the UPF — the downlink actually switched (for N2 the target never reached N4). **This is the first working handover-with-data-continuity on SD-Core.** Phase 4 is GO. |
...
| **D-4** | What GTP-U **SN values** does the UPF accept post-handover? Does it honor End Markers? | **M-1 / P3** | In a PHY-less sim SN values are synthetic; a strict UPF could drop packets → continuity illusory. | iperf3/native flow through a handover; measure loss with/without End Marker and varied synthetic SNs. **PARTIAL 2026-07-04 — DL path switch does NOT take effect; ROOT CAUSE NAILED.** After a completed N2 handover: (a) baseline flow through the source works; (b) uplink from the *target* still reaches the UPF (UL TEID unchanged, accepted); but (c) the UPF keeps sending **downlink to the source** — SMF DL FAR `OuterHeaderCreation` stays `172.17.50.12/TEID 1`; the target `172.17.50.13` **never appears in any N4/PFCP packet** (captured UDP:8805). **Root cause (SMF log, `producer/n1n2_data_handler.go:366`):** `handle HandoverRequestAcknowledgeTransfer failed: align Bit is not zero`. The SMF cannot APER-decode our per-session transfer, so it never extracts the target DL tunnel, never updates the FAR. **Reproduced offline:** `free5gc/aper v1.1.1` (ORBIT) encodes the transfer to `0007c0ac11320d000000c80001`; `omec-project/aper v1.3.2` (SD-Core's SMF) decodes those exact bytes → `align Bit is not zero`. The struct definitions/tags are **identical** across the two libraries, so it is a genuine **aper *implementation* divergence** (byte-alignment of this transfer's bit layout). **Conformance verdict (independent oracle, 2026-07-04):** decoded our exact bytes with `pycrate` (spec-derived 3GPP NGAP ASN.1 + canonical APER, independent of both free5gc and omec) — it decodes cleanly to the right value, **re-encodes to the identical bytes**, and its canonical encode of the same abstract value from scratch is **also identical**. So **ORBIT's encoding IS X.691-conformant — our bytes are the canonical APER — and omec's SMF decoder is the non-conformant party** (it rejects spec-valid canonical APER; its aper fork does a spurious byte-align). This is a genuine **conformance bug in SD-Core**, uniquely surfaced by ORBIT (gnbsim never ran handover). **Do NOT change our (conformant) encoding** — that would break us against conformant cores. **Exact defect (2026-07-04):** omec's `HandoverRequestAcknowledgeTransfer.DLForwardingUPTNLInformation` is missing the `optional` tag (TS 38.413 §9.3.4.11 defines it OPTIONAL), so omec treats a spec-optional field as mandatory. **Fixed via opt-in `sdcore` quirk** (`HandoverAckForwardingMandatory`, §5(i)): emit the transfer with that field present, byte-identical to omec's own encoder, no omec dep. Live-verified — the decode error is gone and the SMF now switches the DL FAR *address* to the target. **But a SECOND, independent SD-Core bug then blocks continuity:** the SMF assigns the target FAR **TEID 0** and never programs the UPF (target never appears on N4/PFCP). Full write-up + both findings: `docs/interop/sdcore.md`. So mobility *signalling* is fully proven; user-plane *continuity* is blocked by two distinct SD-Core handover bugs — the first now worked around, the second open (upstream). |
...
| **D-11** | For the conformance decode path, **`free5gc` vs `omec-project` types** — which byte-identically matches SD-Core output? | before P6 | The omec fork is what SD-Core field-uses; conformance needs exact decode of core-emitted messages. | Capture live SD-Core NGAP; decode with both; diff type coverage + padding handling. **RESOLVED 2026-07-06 — free5gc adequate.** free5gc's `ngap`/`nas` has decoded SD-Core-emitted NGAP across every phase (NG Setup Response, DL NAS Transport, Auth Request, Initial Context Setup, PDU Session Resource Setup Request, HandoverRequest/Command, PathSwitchRequestAcknowledge, UE Context Release). The *one* known free5gc↔omec divergence is **encode-side and type-specific** (the N2 `HandoverRequestAcknowledgeTransfer` missing-`optional` bug, `docs/interop/sdcore.md`), not a decode incompatibility. So Phase 6 decodes with free5gc; adopt omec types only if a specific core-emitted message ever fails. No omec dependency added. |
