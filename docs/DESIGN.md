...
> **About this document.** ORBIT is a standalone project. It was originally scoped
> as "Release 2" of [the-earlier-project](https://github.com/aether-gui/the-earlier-project) (the
> UERANSIM-based 5G twin) and inherits that plan; it now lives and evolves on its
> own. Where this document says "the-earlier-project," it means that sibling project — the
> existing scenario-driven twin ORBIT interoperates with but does not depend on.
> Reuse of the-earlier-project's `internal/*` packages therefore means **copy, or promote the
> package out of `internal/`** (Go forbids cross-module `internal` imports), not a
> direct import — a small caveat on decision §5(b).

---

## 1. Vision

ORBIT is a native Go engine that speaks the real 5G SA control and user planes to a live core — NGAP over SCTP on N2, NAS-5GS tunneled inside it, and GTP-U on N3 — with the radio replaced by a software model instead of an RF PHY. the-earlier-project wrapped UERANSIM behind a Go orchestrator; ORBIT deletes the UERANSIM dependency and owns the protocol engine end to end, so we can do the things UERANSIM structurally cannot:
...
> **Wire trap — NGAP PPID is the integer 60 (0x0000003C), not `0x3c000000`.** `0x3c000000` (= 1,006,632,960) is the *big-endian byte-order rendering* seen in packet captures. If the SCTP `sndinfo.ppid` is set to that value the AMF drops the association. Set PPID = 60; assert it in the Phase-0 NG Setup smoke test.
...
```
                        the-earlier-project (single Go binary; roles selectable)
┌──────────────────────────────────────────────────────────────────────────────┐
...
**Relationship to the the-earlier-project orchestrator.** the-earlier-project's `internal/twin` (`twin.go`, `run.go`) is deeply UERANSIM-specific — `docker exec`, `nr-cli`, per-cell container management. We **keep and extend** the portable packages — `internal/scenario`, `internal/config`, `internal/traffic` — and **replace `twin`** with the new engine behind an `EngineBackend` interface, so UERANSIM stays optionally swappable. **Correction from the draft:** the "contiguous-IMSI" and "one-`nr-ue`-process-per-cell" constraints exist **only as doc comments** (`scenario.go` line 7 and the comment above `validate()`), **not as executable checks** — `validate()` (lines 150–208) enforces cell/NCI/gnbId uniqueness, 15-digit numeric IMSIs, profile/cell references, and duration parsing, and nothing else. So the ORBIT task is to **correct the misleading comments**, not "relax validators," and to **re-derive what actually blocks the-earlier-project scenarios on the native engine** (§6 Phase 2). **the-earlier-project single-slice scenarios must load and run unchanged against the ORBIT engine** (a phase exit criterion; multi-slice is a schema extension, not the-earlier-project compat — see §5(g)).
...
| SCTP transport | **`ishidawataru/sctp`** (fork+pin) · Apache-2.0/BSD-3 | De-facto standard (gnbsim/PacketRusher/free5gc). No tags, ~23 open issues → **fork under our org, pin a commit** from day one. CGO/Linux-only. |
| GTP-U (N3) Stage 1 | **`wmnsk/go-gtp`** (fork/extend) · **license TBD — verify (MIT vs Apache-2.0) before building on it** | Pure-Go, no kernel module → CI-safe. *Experimental, ~25% coverage, no `0x85`/QFI → build the 12-byte header + PDU Session Container + End Marker on top (2–3 wk spike, Phase 1b).* |
...
### (b) Engine vs Release-1-orchestrator relationship — **EXTEND the portable core, REPLACE `twin`. Decided.**
Keep `internal/scenario`, `internal/config`, `internal/traffic`; introduce an `EngineBackend` interface so the-earlier-project's UERANSIM path stays swappable and ORBIT's native engine is the new default. Do **not** reuse `twin.Twin`/`twin.Run` (docker/nr-cli-bound). **The "relax the contiguous-IMSI / one-process-per-cell validators" task in the draft is void** — those checks do not exist in `validate()`; only misleading doc comments do. Replace those comments; independently identify what genuinely blocks the-earlier-project scenarios (config single-slice assumption, TrafficProfile mapping, fleet expansion). **Exit criterion:** existing the-earlier-project single-slice scenarios (`poc.yaml`, `multicell.yaml`, `scale-run.yaml`) run unchanged on the ORBIT engine.

### (c) RRC & XnAP: wire-encode now, or in-process stub? — **STUB in-process first. Decided; highest-leverage call, single-point-gated.**
Because the AMF passes RRC containers **opaquely** and never touches XnAP, the NGAP portion of a full handover against the *real* core can be validated with RRC and XnAP as **Go structs on channels**. Full TS 38.331 UPER / TS 38.423 APER codecs become mandatory only for inter-vendor interop or running two the-earlier-project gNBs as *separate processes*. **This decision rests entirely on D-3, and D-3's framing is refined:** it must confirm (1) the AMF is opaque to *RRC bytes*, **and** (2) `free5gc/ngap` (or the omec fork) correctly encodes the NGAP-level Source-to-Target Transparent Container that *wraps* the RRC blob, because our own target-side decode depends on it. Budget a contingency (partial NGAP-container encoding work) if (2) surprises.
...
**Verification:** unit-CI green headless; `the-earlier-project serve` starts, CLI reaches it over Connect; an SCTP association opens to the AMF's N2 port and **NG Setup** is exchanged (no UE yet), PPID asserted = 60.
**Exit:** NG Setup succeeds against a live core AMF; substrate compiles and is observable; SCTP CGO/Linux-only constraint documented.
...
**Tasks:** NGAP procedure FSM (NG Setup → Initial UE Message → DL/UL NAS Transport → Initial Context Setup → PDU Session Resource Setup); NAS-MM FSM (Registration → **5G-AKA** challenge/response → Security Mode → Registration Accept); one 5GSM PDU Session Establishment (signaling to `PDU SESSION ACTIVE`); **5G-AKA + Milenage + full key hierarchy** + NAS integrity/ciphering at **NEA0/NIA2**; **null-scheme SUCI only** (SD-Core default accepts it — D-8); expose `Register/Deregister/Status` unary RPCs + `StateStream`. Golden-bytes fixture for the **deterministic** registration/NGAP messages (§5f).
...
3. Wire the `EngineBackend` behind `internal/scenario`; **correct the misleading doc comments** (no validator relaxation needed — §5b); confirm the real the-earlier-project blockers and fix them.
4. **Slicing schema extension (§5g):** add per-UE Requested NSSAI + per-session DNN/S-NSSAI to the scenario schema; enable **multi-PDU-session per UE** (up to 15, TS 23.501 §5.8.2.2). Keep single global `Core.Slice`/`Core.DNN` as the default so the-earlier-project scenarios still parse.
5. Introduce **Mode B** (shared-TUN demux) so scale does not require 10k TUN devices.
**Verification:** the-earlier-project **single-slice** `multicell.yaml`/`scale-run.yaml` load and run on the ORBIT engine; register N UEs across ≥2 gNBs on the live core (checkpoint: the SD-Core-validated **5,000 UEs @ 10 attach/s**); **sim-capacity** (goroutine/RSS scaling to 10k) measured separately against the mock AMF; memory ≈ the-earlier-project's ~2 MB/UE order or better.
**Exit:** stable multi-cell, multi-session fleet on the real core; the-earlier-project single-slice backward-compat proven; actor model chosen on D-6 evidence; Mode B available for scale.
...
**Exit:** a named mobility test target (SD-Core, free5gc, or mock-AMF-degraded) with D-1/D-4 evidence.
...
|---|---|---|---|---|
| **D-1** | Does a live core (SD-Core; else free5gc) actually complete **N2** handover? | **M-1, before P3** | gnbsim never exercised it; omec *declares* support (v3.1.0) but SD-Core 1.2 had a context-persistence bug fixed only in 1.3. **This gates the mobility value prop, not just the FSM.** | Minimal `HandoverRequired` at the live AMF; confirm `HandoverCommand` + SMF/UPF N4 modification + N3 switch. Run against SD-Core **and** the pre-staged free5gc. |
...
| **D-6** | Goroutine-per-UE vs worker-pool at 10k+? | **early P2, before building the actor model** | ~10k UEs × ~3 goroutines × ~8 KB ≈ 240 MB before state; bursty attach storms stress the scheduler. **Must precede the architecture commit.** | Attach 10k vs a **mock AMF**; measure RSS + p99 attach latency. |
...
| **D-8** | Default core **auth method** (5G-AKA vs EAP-AKA') and null-SUCI acceptance? | P0 | Confirms 5G-AKA + null SUCI is the critical path (keeps Profile A/B off the keystone). | Inspect deployed SD-Core config; attach with null SUCI. |
...
|---|---|---|
| **Live core cannot do handover** — findings say SD-Core likely can't (gnbsim never ran it; 1.2 bug; 1.3 "groundwork"). | The headline mobility differentiator collapses to mock-AMF signaling. | **Milestone Gate M-1** is a first-class go/no-go on the *value prop*: D-1/D-4 against SD-Core **and a pre-staged free5gc**; explicit degraded fallback (mock-AMF validation) with qualified external claims. Not a mere risk-table row. |
...
| **Phantom validator task** — draft budgeted work to "relax" checks that don't exist. | Misspent effort; misread foundation undercuts the-earlier-project-compat confidence. | **Corrected (§5b):** fix misleading doc comments; independently re-derive real the-earlier-project blockers (config single-slice, traffic mapping, fleet expansion). |
...
| **Slicing needs unflagged schema work; "run unchanged" overclaimed.** | Multi-session/slicing silently blocked; the-earlier-project-compat claim too broad. | **§5g:** per-UE NSSAI + per-session DNN/S-NSSAI is an explicit Phase-2 task; "run unchanged" scoped to single-slice the-earlier-project scenarios; slicing gets Phase-7 acceptance criteria. |
...
| **SCTP lib fragility** (no tags, ~23 issues, CGO/Linux-only). | Build breakage / cross-platform loss. | **Fork+pin at P0**; document Linux/CGO constraint; D-10 validates streams/multi-homing before scale. |
...
| **Dormant-dependency risk** (`songgao/water`, `wmnsk/go-gtp` license ambiguity). | Silent breakage / license surprise. | **Fork+pin `water` like sctp/go-gtp; verify go-gtp's license at P0 before building the QFI extension.** |
...
**Relevant repo paths:** `/home/ben/repos/bengrewell/the-earlier-project/internal/scenario/scenario.go` (schema to preserve/extend; **doc comments at line 7 and above `validate()` are misleading — no contiguous-IMSI / one-process-per-cell checks exist in `validate()`, lines 150–208; correct the comments, add per-UE NSSAI + per-session DNN/S-NSSAI for §5g**); `/home/ben/repos/bengrewell/the-earlier-project/internal/config/config.go` (**single global `Core.Slice`/`Core.DNN` at lines 38–39, 54–57 — the slicing-extension baseline**); `/home/ben/repos/bengrewell/the-earlier-project/internal/twin/{twin.go,run.go}` (UERANSIM-specific, replaced behind `EngineBackend`); `/home/ben/repos/bengrewell/the-earlier-project/internal/{traffic,config,ueransim,oci}` (traffic/config keep; ueransim/oci become one backend); `/home/ben/repos/bengrewell/the-earlier-project/docs/DESIGN.md` (the-earlier-project design of record); `/home/ben/repos/bengrewell/the-earlier-project/cmd/the-earlier-project/main.go` (CLI entry to re-point at the Connect client).
